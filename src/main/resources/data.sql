...
SELECT CITIES.NAP_CITY_ID, CITIES.NAP_FONETIC, STREETS.NAP_STREET_NAME , STREETS.NAP_STREET_ID FROM CITIES LEFT JOIN STREETS  ON  CITIES.NAP_CITY_ID = STREETS.NAP_CITY_ID WHERE (? is null OR  CITIES.NAP_CITY_ID=?) AND (? is null OR CITIES.NAP_FONETIC = ?) AND (? is null OR STREETS.NAP_STREET_ID=?) AND (? is null OR NAP_STREET_ID=?);



DROP TABLE FORECLOSURE IF EXISTS;
CREATE TABLE FORECLOSURE (CLIENT_ID VARCHAR(9), SUM_FORECLOSURES NUMBER(16) ,FORECLOSURES_BALANCE NUMBER(16), SUM_SEIZED NUMBER(16) , TOTAL_BALANCE NUMBER(16) , MAX_DATE DATE);

INSERT INTO FORECLOSURE VALUES  ('3333333' , 12344,123444,123444,123444,CURRENT_DATE);
